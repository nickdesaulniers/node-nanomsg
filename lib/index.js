...
// we need to keep track of all of the sockets ever created, in order to call
// _stopPollSend and _stopPollReceive on calls to module.term().
var globalSocketList = [];
...
function term () {
    globalSocketList.forEach(function (socket) {
        socket._stopPollSend();
        socket._stopPollReceive();
    });
    return nn.Term();
}

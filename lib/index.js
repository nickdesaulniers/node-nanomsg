 ...
 */
function Socket (domain, type, binding, sender, receiver) {
    this.type = type;
...
function Device(sock1,sock2) {
    ...
    if(sock1 instanceof Socket) {
        ...
        if(sock2 instanceof Socket) {
            ...
            console.log("SOCK1 IS NOT A SOCKET!");
        }
...
function createSocket(type) {
    return createDomainSocket(nn.AF_SP, type);
}

function createRawSocket(type) {
    return createDomainSocket(nn.AF_SP_RAW, type);
}

function createDomainSocket(domain, type) {
    switch(type) {
        case 'req': return new Socket(domain, type, nn.Socket(domain, nn.NN_REQ), true, true);
        case 'rep': return new Socket(domain, type, nn.Socket(domain, nn.NN_REP), true, true);
        case 'pair': return new Socket(domain, type, nn.Socket(domain, nn.NN_PAIR), true, true);
        case 'push': return new Socket(domain, type, nn.Socket(domain, nn.NN_PUSH), true, false);
        case 'pull': return new Socket(domain, type, nn.Socket(domain, nn.NN_PULL), false, true);
        case 'pub': return new Socket(domain, type, nn.Socket(domain, nn.NN_PUB), true, false);
        case 'sub': return new Socket(domain, type, nn.Socket(domain, nn.NN_SUB), false, true);
        case 'bus': return new Socket(domain, type, nn.Socket(domain, nn.NN_BUS), true, true);
        case 'surveyor': return new Socket(domain, type, nn.Socket(domain, nn.NN_SURVEYOR), true, false);
        case 'respondent': return new Socket(domain, type, nn.Socket(domain, nn.NN_RESPONDENT), true, true);
    }
...
exports.createRawSocket = createRawSocket;
exports.symbolInfo = symbolInfo;
exports.symbol = symbol;
exports.term = term;
exports.socket = createSocket;
exports.rawSocket = createRawSocket;
